...
test('1. Проверка статического текста', async ({ page }) => {
  ...
  const staticElement = page.locator('#static-text');
  await expect(staticElement).toContainText('important information');
  await expect(staticElement).not.toContainText('dynamic content');
});
...
test('2. Проверка динамически изменяемого текста', async ({ page }) => {
  ...
  const dynamicElement = page.locator('#dynamic-text');
  await expect(dynamicElement).toContainText('Initial dynamic text');
  await page.locator('#change-text').click();
  await expect(dynamicElement).toContainText('Text was changed at');
  await page.locator('#add-part').click();
  await expect(dynamicElement).toContainText('(additional part)');
});
...
test('3. Проверка списка элементов', async ({ page }) => {
  ...
  const listElement = page.locator('#item-list');
  await expect(listElement).toContainText('Item 1: Basic');
  await expect(listElement).toContainText('Intermediate');
  await page.locator('#add-item').click();
  await expect(listElement).toContainText('New added item');
});
...
test('4. Проверка скрытого/отображаемого текста', async ({ page }) => {
  ...
  await expect(hiddenElement).toContainText('special content');
...
test('5. Проверка частичного совпадения в длинном тексте', async ({ page }) => {
  ...
  const partialElement = page.locator('#partial-text');
  await expect(partialElement).toContainText('quick brown fox');
  await expect(partialElement).toContainText('lazy dog');
  await expect(partialElement).toContainText('all letters of the English alphabet');
  await expect(partialElement).not.toContainText('all letters of the Russian alphabet');
});
