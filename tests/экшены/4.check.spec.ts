...
test.describe('Комплексное тестирование формы с чекбоксами', () => {
  ...
  test('Полное заполнение формы с проверкой состояний', async ({ page }) => {
    ...
    await page.getByRole('checkbox', { name: 'Я принимаю условия соглашения' }).check();
    ...
    // Радио-кнопки
    await page.getByRole('radio', { name: 'Почта России' }).check();

    // Кастомный элемент
    await page.locator('.tos-container').scrollIntoViewIfNeeded();
    await page.getByRole('checkbox', { name: 'Я прочитал и согласен с условиями' }).check();
